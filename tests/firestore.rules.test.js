 ...
 *
 * Verifies the security rule logic for the Türkan Abla platform.
 * Uses a rule-evaluation helper that mirrors the Firestore rules engine,
 * testing the same security properties enforced in production.
 *
 * The rules enforce:
 ...
 * dependency. The TypeScript version (firestore.rules.test.ts) is the canonical
 * source; this file is the runnable equivalent for CI environments without Java.
 */
...
const helpers = {
  isAuthenticated: (ctx) => ctx.auth !== null,
  userId: (ctx) => ctx.auth?.uid ?? '',
  userRole: (ctx) => ctx.auth?.token.role ?? '',
  userVendorId: (ctx) => ctx.auth?.token.vendorId ?? '',
  isOwner: (ctx, resourceUid) => helpers.userId(ctx) === resourceUid,
  isVendor: (ctx) => helpers.userRole(ctx) === 'vendor',
  isCustomer: (ctx) => helpers.userRole(ctx) === 'customer',
  isStaff: (ctx) => helpers.userRole(ctx) === 'staff',
  isAdmin: (ctx) => ctx.auth?.token.role === 'admin',
};

// ---------------------------------------------------------------------------
// Rule definitions – JavaScript translation of firestore.rules conditions.
// Each returns true if the operation is ALLOWED.
// ---------------------------------------------------------------------------

const rules = {
  /** /users/{userId} – read */
  usersRead: (ctx, userId) =>
    helpers.isAuthenticated(ctx) &&
    (helpers.isOwner(ctx, userId) || helpers.isAdmin(ctx)),

  /** /users/{userId} – create */
  usersCreate: (ctx, userId, newData) =>
    helpers.isAuthenticated(ctx) &&
    helpers.isOwner(ctx, userId) &&
    newData.role === 'customer',

  /** /users/{userId} – update/delete */
  usersWrite: (ctx, userId) => helpers.isOwner(ctx, userId),

  /** /vendors/{vendorId}/bookings/{bookingId} – read */
  vendorBookingsRead: (ctx, bookingData) =>
    helpers.isAuthenticated(ctx) &&
  ...
  suppressionsRead: (ctx) => helpers.isAdmin(ctx),

  /** /messageLog/{id} – write */
  messageLogWrite: (ctx) => helpers.isAdmin(ctx),

  /** /messageLog/{id} – read */
  messageLogRead: (ctx) => helpers.isAdmin(ctx),

  /** /consentFlags/{userId} – read */
  consentFlagsRead: (ctx, userId) =>
    helpers.isAdmin(ctx) || helpers.isOwner(ctx, userId),

  /** /consentFlags/{userId} – write */
  consentFlagsWrite: (ctx) => helpers.isAdmin(ctx),

  /** /reviews/{reviewId} – create */
  reviewsCreate: (ctx, newData) =>
    helpers.isCustomer(ctx) && newData.customerId === helpers.userId(ctx),

  /** /loyaltyLedger/{id} – write */
  loyaltyLedgerWrite: (ctx) => helpers.isAdmin(ctx),

  /** /commissionLedger/{id} – write */
  commissionLedgerWrite: (ctx) => helpers.isAdmin(ctx),
...
function customerCtx(uid) {
  return { auth: { uid, token: { uid, role: 'customer' } } };
}

function vendorCtx(uid, vendorId) {
...
describe('Firestore Security Rules – IDOR prevention', () => {
  ...
  it('customer can create a booking for themselves', () => {
    const ctx = customerCtx('customer1');
    const newData = { customerId: 'customer1', vendorId: 'vendor1' };
  ...
  it('customer cannot create booking for a different vendor than the doc path', () => {
    ...
    const newData = { customerId: 'customer1', vendorId: 'vendor2' };
